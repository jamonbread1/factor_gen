...
def run(train,valid,test,cfg=None):
    ...
    screened=[]
    for c in candidates:
        values=candidate_values(valid,c); s=cheap_screen(values,valid["target"].to_numpy(float),valid["eob"].to_numpy(),float(cfg.get("cheap_threshold",.01)))
        if np.isfinite(s): screened.append((s,c,values))
    screened.sort(key=lambda z:z[0],reverse=True); exact_limit=int(cfg.get("validation_top_k",200)); ranked=[]
    for i,(cheap,c,values) in enumerate(screened[:exact_limit],1):
        r=fast_factor_report(valid,values,"target",float(cfg.get("ic_threshold",.03)),float(cfg.get("min_positive_window_ratio",.60))); ranked.append((r.get("mean_ic",-np.inf),c,r,values))
    ...
    evaluated=[]
    for item in frozen:
        tf=candidate_frame(test,item["candidate"]); te=fast_factor_report(tf,tf["factor_value"].to_numpy(float),"target",float(cfg.get("ic_threshold",.03)),float(cfg.get("min_positive_window_ratio",.60))) if len(tf) else {"status":"INSUFFICIENT_DATA"}
        evaluated.append({**item,"test":te,"signal":signal_backtest_stats(tf,"factor_value",signal_cfg) if len(tf) else {}})
