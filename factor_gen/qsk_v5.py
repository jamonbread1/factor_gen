...
LOG = logging.getLogger("factor_gen.qsk_v51")
...
def make_panel(df: pd.DataFrame, horizon: int = 5) -> pd.DataFrame:
    ...
    logv = np.log1p(x["volume"].clip(lower=0))
...
def split_by_time(df, train_ratio=.60, valid_ratio=.20):
    ...
    if a<30 or b<=a or b>=len(dates): raise ValueError("时间切分后样本不足，请增加历史K线长度")
...
def candidate_values(frame,candidate:Candidate):
    ...
    return np.asarray(evaluate_spec(frame,spec),dtype=np.float32)


def candidate_frame(frame,candidate):
    out=frame.copy(); out["factor_value"]=candidate_values(frame,candidate)
...
def orient_candidate(candidate, mean_ic):
    if not np.isfinite(mean_ic) or mean_ic >= 0: return candidate
    w=-candidate.weights
    return replace(candidate, weights=w, expression=f"(-1*{candidate.expression})")
...
def run(train,valid,test,cfg=None):
    cfg=cfg or {}; out=Path(cfg.get("output_dir","generated_v51")); out.mkdir(parents=True,exist_ok=True)
    horizon=int(cfg.get("horizon",5)); signal_cfg=SignalConfig(horizon=horizon,buy_threshold=float(cfg.get("buy_threshold",.03)),sell_threshold=float(cfg.get("sell_threshold",-.03)),flat_threshold=float(cfg.get("flat_threshold",.01)),stop_loss=float(cfg.get("stop_loss",-.04)),take_profit=float(cfg.get("take_profit",.08)))
    ...
    # Stage 1: cheap vectorized validation. Only a small shortlist receives exact daily IC.
    screened=[]
    for i,c in enumerate(candidates,1):
        values=candidate_values(valid,c); score=cheap_screen(values,valid["target"].to_numpy(float),valid["eob"].to_numpy(),float(cfg.get("cheap_threshold",.01)))
        if np.isfinite(score): screened.append((score,c,values))
    screened.sort(key=lambda z:z[0],reverse=True)
    exact_limit=int(cfg.get("validation_top_k",200)); ranked=[]
    for i,(cheap,c,values) in enumerate(screened[:exact_limit],1):
        report=fast_factor_report(valid,values,"target",float(cfg.get("ic_threshold",.03)),float(cfg.get("min_positive_window_ratio",.60)))
        ranked.append((report.get("mean_ic",-np.inf),c,report,values))
    ...
    for _,c,vr,v in ranked:
        ...
        folds=rolling_fold_scores(valid,vf["factor_value"].to_numpy(float),"target",int(cfg.get("walk_forward_folds",4)),float(cfg.get("ic_threshold",.03)))
        frozen.append({"candidate":c,"train":tr,"valid":vr,"walk_forward":folds}); frozen_values.append(vf["factor_value"].to_numpy(float))
        if len(frozen)>=int(cfg.get("final_top_k",20)): break
    if not frozen: raise RuntimeError("没有通过候选验证与相关性门控的候选因子")
    evaluated=[]
    for item in frozen:
        tf=candidate_frame(test,item["candidate"]); values=tf["factor_value"].to_numpy(float)
        te=fast_factor_report(tf,values,"target",float(cfg.get("ic_threshold",.03)),float(cfg.get("min_positive_window_ratio",.60))) if len(tf) else {"status":"INSUFFICIENT_DATA"}
        sig_stats=signal_backtest_stats(tf,"factor_value",signal_cfg) if len(tf) else {}
        evaluated.append({**item,"test":te,"signal":sig_stats})
    # Select by validation only. Test remains OOS.
    best=sorted(evaluated,key=lambda x:(x["valid"].get("mean_ic",-np.inf),x["valid"].get("ic_ir",-np.inf),x["valid"].get("median_ic",-np.inf)),reverse=True)[0]
    c=best["candidate"]; report=best["test"]; factor_path=out/"weekly_signal_factor_v51.py"; _export_factor(c,best["valid"],factor_path,signal_cfg)
    model_path=out/"ai_transformer_v51.pt"; model_saved=_save_transformer(model,model_path)
    payload={"version":"5.1-weekly-signal","horizon":horizon,"status":report.get("status"),"selection_basis":"validation_only","expression":c.expression,"train":best["train"],"valid":best["valid"],"walk_forward":best["walk_forward"],"test":report,"signal":best["signal"],"model":str(model_path) if model_saved else None,"factor":str(factor_path),"search":{"candidates":len(candidates),"cheap_screened":len(screened),"exact_validated":min(exact_limit,len(screened))},"top_candidates":[{"expression":x["candidate"].expression,"valid":x["valid"],"test":x["test"],"signal":x["signal"]} for x in evaluated]}
    (out/"factor_report_v51.json").write_text(json.dumps(payload,ensure_ascii=False,indent=2,default=str),encoding="utf-8")
    LOG.info("FINAL %s horizon=%d testIC=%.5f mean=%.5f IR=%.3f active=%.2f",report.get("status"),horizon,report.get("ic",np.nan),report.get("mean_ic",np.nan),report.get("ic_ir",np.nan),best["signal"].get("active_ratio",np.nan))
    return payload
