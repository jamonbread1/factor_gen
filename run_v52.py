...
def main():
    p = argparse.ArgumentParser(description="factor_gen V5.2 regime-aware weekly trend signal miner")
    p.add_argument("--config", default="config_v52.yaml")
    ...
    train, valid, test = split_by_time(panel, float(cfg.get("train_ratio", .60)), float(cfg.get("valid_ratio", .20)), int(cfg.get("embargo", cfg.get("horizon", 5))))
    result = run(train, valid, test, cfg)
    print("\n===== FACTOR GEN V5.2 REGIME-AWARE SIGNAL =====")
    ...
    print("RECOVERY         :", result.get("recovery"))
    print("REGIME TEST      :", result.get("regime_test"))
